...
- **History パネル: 初期状態エントリの選択**: 探索ステータスの History リストの末尾に、
  一切イベントが記録されていない「初期状態」（全ノート未読）を表す常設の選択可能な
  エントリ（"Initial state"）を追加。クリックすると全ノートが未読の状態として rewind
  表示される（[ADR-0014](decisions/ADR-0014-node-exploration-status-persistence.md) 改訂、
  REQ-EXPLORE-003）。

### 変更

- **History パネル: タイムゾーン表示を廃止**: History リストの各エントリのタイムスタンプ表示を、
  `(UTC±HH:MM)` 付きの表記から日時のみの表記に変更（ノートページの読了日時表示と統一）。
