...
- `alias` 指定が無い場合の表示テキストは、解決先ノートの **タイトル**（2.4節）を使う
  （生の `[[note]]` の `note` 部分、すなわち ID をそのまま表示テキストにはしない）。
  ID はリンクの記述・解決・URL に使う識別子であり、タイトルは表示専用という
  [ADR-0009](../decisions/ADR-0009-note-id-title-separation.md) の原則をリンク表示にも
  適用したものである。
